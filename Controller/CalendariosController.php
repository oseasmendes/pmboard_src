       ...
       if (!empty($keyword['mesnome'])) {
                    ...
                    $calendarios = $this->paginate($this->Calendarios);
                    } elseif (!empty($keyword['calendariosplano_id'])) 
                            {
                                ...
                                $calendarios = $this->paginate($this->Calendarios);
                            } elseif (!empty($keyword['datainicio']) && !empty($keyword['datafim'])) 
                            {
                                $this->paginate = [
                                'contain' => ['Calendariosplanos'],
                                //'conditions' => ['data >= '=> $keyword['datainicio']],     
                               'conditions' => ['Calendarios.data >= '=> $keyword['datainicio'],'data <= '=> $keyword['datafim']],     
                                'order' => [                                
                                 'Calendarios.data' => 'asc'
                                    ]            
                                ];
                                $calendarios = $this->paginate($this->Calendarios);
                            } else {
                                $this->paginate = [
                                 'contain' => ['Calendariosplanos'],
                                 'conditions' => ['Calendarios.data >= '=> $dataatual],     
                                  'order' => [
                                'Calendarios.id' => 'asc'
