 ...
 * Upload component
 */
class UploadprojetosprodutosComponent extends Component
{
	public $max_files = 3;
	...
	public function upload($data,$galeria) 
	{
		...
		return $this->_registry->getController()->redirect(['controller' => 'projetosprodutosimgs','action' => 'add',$galeria]);
		...
		foreach ($data as $file) 
		{
			...
			$dir = WWW_ROOT.'img'.DS.'projetosprodutos'.DS.$galeria;
			//var_dump($dir);
			$type_allowed = array('png','jpg','jpeg','gif','pptx','xls','xlsx','pdf','doc','docx');
					...
					elseif (is_uploaded_file($file_tmp_name)) 
					{
								...
								$file_db = TableRegistry::get('Projetosprodutosimgs');
								...
								//var_dump($filename);
								$entity->projetosproduto_id = $galeria;
								$file_db->save($entity);
					...
					return $this->_registry->getController()->redirect(['controller'=>'projetosprodutos','action' => 'view',$galeria]);	
					}
