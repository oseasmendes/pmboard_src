 ...
 * Upload component
 */
class UploadprojetosComponent extends Component
{
	public $max_files = 3;
	...
	public function upload($data,$galeria) 
	{
		...
		return $this->_registry->getController()->redirect(['controller' => 'projetosimgs','action' => 'add',$galeria]);
		...
		foreach ($data as $file) 
		{
			...
			$dir = WWW_ROOT.'img'.DS.'projetos'.DS.$galeria;
			//var_dump($dir);
			$type_allowed = array('png','jpg','jpeg','gif','pptx','xls','xlsx','pdf','doc','docx');
					...
					//->original:	return $this->_registry->getController()->redirect(['action'=>'add');
						...
						return $this->_registry->getController()->redirect(['controller' => 'projetosimages','action' => 'add',$galeria]);
					...
					elseif (is_uploaded_file($file_tmp_name)) 
					{
								...
								$file_db = TableRegistry::get('Projetosimages');
								$entity = $file_db -> newEntity();
								$entity->image = $filenameoriginal;
								$entity->pathimage = $dir;
								$entity->imagemid = $filenameid;
								//var_dump($filename);
								$entity->projeto_id = $galeria;
								$file_db->save($entity);
					...
					return $this->_registry->getController()->redirect(['controller'=>'projetos','action' => 'view',$galeria]);	
					}
