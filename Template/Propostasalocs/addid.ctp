...
<div class="propostasalocs form large-9 medium-8 columns content">
    <?= $this->Form->create($propostasaloc) ?>
            <?php $paramid = $_POST['propalocacao']; ?>
            <?php $this->proposta_id = $paramid; ?>
    <fieldset>
        <legend><?= __('Add Propostasaloc') ?></legend>
        <?php            
            echo $this->Form->control('proposta_id', array('default'=>$paramid,'disabled' => 'disabled','value'=>$paramid,'class'=>['class'=> 'form-control']));
