...
<div class="todoseventos form large-9 medium-8 columns content">
    <?= $this->Form->create($todosevento) ?>            
            <?php $paramid = $_POST['Todoevento'];
            //var_dump($tktid); ?>
            <?= $this->todo_id = $paramid; ?>
    <fieldset>
        <legend><?= __('Add Todosevento') ?></legend>
        <?php
            echo $this->Form->control('historico');
            echo $this->Form->control('data', ['empty' => true]);            
            echo $this->Form->control('todo_id', array('default'=>$paramid,'disabled' => 'disabled','value'=>$paramid)); //inserido esse parametro para receber projeto_id  
