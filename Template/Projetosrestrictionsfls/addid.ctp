...
<div class="projetosrestrictionsfls form large-9 medium-8 columns content">
    <?= $this->Form->create($projetosrestrictionsfl) ?>
            ...
            <?php $this->projetosrestriction_id = $paramid; ?>   
    <fieldset>
        <legend><?= __('Add Projetosrestrictionsfl') ?></legend>
        <?php
            //echo $this->Form->control('projetosrestriction_id', ['options' => $projetosrestrictions, 'empty' => true]);
             echo $this->Form->control('projetosrestriction_id', array('default'=>$paramid,'disabled' => 'disabled','value'=>$paramid,'class'=>['class'=> 'form-control']));
