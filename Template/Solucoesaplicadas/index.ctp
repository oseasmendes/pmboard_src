 ...
 * @var \App\Model\Entity\Solucoesaplicada[]|\Cake\Collection\CollectionInterface $solucoesaplicadas
 */
?>
...
<div class="solucoesaplicadas index large-9 medium-8 columns content">
    <h3><?= __('Solucoesaplicadas') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col"><?= $this->Paginator->sort('id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('descricao') ?></th>
                <th scope="col"><?= $this->Paginator->sort('solucoestipo_id') ?></th>
        ...
        <tbody>
            <?php foreach ($solucoesaplicadas as $solucoesaplicada): ?>
            <tr>
                ...
                <td><?= h($solucoesaplicada->ativo) ?></td>
                <td><?= h($solucoesaplicada->created) ?></td>
                <td><?= h($solucoesaplicada->modified) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $solucoesaplicada->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $solucoesaplicada->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $solucoesaplicada->id], ['confirm' => __('Are you sure you want to delete # {0}?', $solucoesaplicada->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
