<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Severidade $severidade
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Severidade'), ['action' => 'edit', $severidade->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Severidade'), ['action' => 'delete', $severidade->id], ['confirm' => __('Are you sure you want to delete # {0}?', $severidade->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Severidades'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Severidade'), ['action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="severidades view large-9 medium-8 columns content">
    <h3><?= h($severidade->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Descricao') ?></th>
            <td><?= h($severidade->descricao) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($severidade->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Nota') ?></th>
            <td><?= $this->Number->format($severidade->nota) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($severidade->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($severidade->modified) ?></td>
        </tr>
    </table>
</div>
