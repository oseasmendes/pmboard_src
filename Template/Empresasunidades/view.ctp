<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Empresasunidade $empresasunidade
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Empresasunidade'), ['action' => 'edit', $empresasunidade->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Empresasunidade'), ['action' => 'delete', $empresasunidade->id], ['confirm' => __('Are you sure you want to delete # {0}?', $empresasunidade->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Empresasunidades'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Empresasunidade'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Empresas'), ['controller' => 'Empresas', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Empresa'), ['controller' => 'Empresas', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="empresasunidades view large-9 medium-8 columns content">
    <h3><?= h($empresasunidade->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Descricao') ?></th>
            <td><?= h($empresasunidade->descricao) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Sigla') ?></th>
            <td><?= h($empresasunidade->sigla) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Empresa') ?></th>
            <td><?= $empresasunidade->has('empresa') ? $this->Html->link($empresasunidade->empresa->fantasia, ['controller' => 'Empresas', 'action' => 'view', $empresasunidade->empresa->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($empresasunidade->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($empresasunidade->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($empresasunidade->modified) ?></td>
        </tr>
    </table>
</div>
