<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Ocorrenciastipo $ocorrenciastipo
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $ocorrenciastipo->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $ocorrenciastipo->id)]
            )
        ?></li>
        <li><?= $this->Html->link(__('List Ocorrenciastipos'), ['action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('List Ocorrencias'), ['controller' => 'Ocorrencias', 'action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('New Ocorrencia'), ['controller' => 'Ocorrencias', 'action' => 'add']) ?></li>
    </ul>
</nav>
<div class="ocorrenciastipos form large-9 medium-8 columns content">
    <?= $this->Form->create($ocorrenciastipo) ?>
    <fieldset>
        <legend><?= __('Edit Ocorrenciastipo') ?></legend>
        <?php
            echo $this->Form->control('descricao');
        ?>
    </fieldset>
    <?= $this->Form->button(__('Submit')) ?>
    <?= $this->Form->end() ?>
</div>
