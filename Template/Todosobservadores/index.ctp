<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Todosobservadore[]|\Cake\Collection\CollectionInterface $todosobservadores
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('New Todosobservadore'), ['action' => 'add']) ?></li>
        <li><?= $this->Html->link(__('List Todos'), ['controller' => 'Todos', 'action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('New Todo'), ['controller' => 'Todos', 'action' => 'add']) ?></li>
        <li><?= $this->Html->link(__('List Participantes'), ['controller' => 'Participantes', 'action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('New Participante'), ['controller' => 'Participantes', 'action' => 'add']) ?></li>
    </ul>
</nav>
<div class="todosobservadores index large-9 medium-8 columns content">
    <h3><?= __('Todosobservadores') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col"><?= $this->Paginator->sort('id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('todo_id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('participante_id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('created') ?></th>
                <th scope="col"><?= $this->Paginator->sort('modified') ?></th>
                <th scope="col"><?= $this->Paginator->sort('papel') ?></th>
                <th scope="col" class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($todosobservadores as $todosobservadore): ?>
            <tr>
                <td><?= $this->Number->format($todosobservadore->id) ?></td>
                <td><?= $todosobservadore->has('todo') ? $this->Html->link($todosobservadore->todo->id, ['controller' => 'Todos', 'action' => 'view', $todosobservadore->todo->id]) : '' ?></td>
                <td><?= $todosobservadore->has('participante') ? $this->Html->link($todosobservadore->participante->nome, ['controller' => 'Participantes', 'action' => 'view', $todosobservadore->participante->id]) : '' ?></td>
                <td><?= h($todosobservadore->created) ?></td>
                <td><?= h($todosobservadore->modified) ?></td>
                <td><?= h($todosobservadore->papel) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $todosobservadore->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $todosobservadore->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $todosobservadore->id], ['confirm' => __('Are you sure you want to delete # {0}?', $todosobservadore->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(['format' => __('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')]) ?></p>
    </div>
</div>
