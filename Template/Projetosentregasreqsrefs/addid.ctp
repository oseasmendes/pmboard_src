...
<div class="projetosentregasreqsrefs form large-9 medium-8 columns content">
    ...
    <fieldset>
        <legend><?= __('Novo Requisito') ?></legend>
        <?php
            //echo $this->Form->control('projetosentregasreq_id', ['options' => $projetosentregasreqs, 'empty' => true,'class'=>['class'=> 'form-control']]);
                        
            echo $this->Form->control('projetosentregasreq_id', array('default'=>$paramid,'disabled' => 'disabled','label' => 'Requisito Principal' ,'value'=>$paramid,'class'=>['class'=> 'form-control'])); //inserido esse parametro para receber projeto_id
